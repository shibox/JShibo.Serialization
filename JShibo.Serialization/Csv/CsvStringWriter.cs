...
namespace JShibo.Serialization.Csv
{
    public class CsvStringWriter
    {
        ...
        internal Type[] types;
        internal Serialize<CsvStringWriter>[] sers;
        ...
        public CsvStringWriter(int capacity)
            : this(capacity, SerializerSettings.Default)
        {
        }

        public CsvStringWriter(char[] buffer)
            : this(buffer, SerializerSettings.Default)
        {
        }

        public CsvStringWriter(ref char[] buffer)
            : this(ref buffer, SerializerSettings.Default)
        {
        }

        public CsvStringWriter(char[] buffer, SerializerSettings set)
        {
            _buffer = buffer;
            sets = set;
        }

        public CsvStringWriter(ref char[] buffer, SerializerSettings set)
        {
            _buffer = buffer;
            sets = set;
        }

        public CsvStringWriter(int capacity, SerializerSettings set)
        {
            _buffer = new char[capacity];
            sets = set;
        }

        internal CsvStringWriter(CsvStringWriter stream)
        {
        ...
        {
            var start = position;
            fixed (char* pdst = &_buffer[position],psrc = headers)
            {
                var tdst = pdst;
